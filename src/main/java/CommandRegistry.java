            ...
            (scanner, system) -> {
                ...
                switch (choice) {
                    case 1:
                        ...
                        break;
                    case 2:
                        String performer = ConsoleUtils.promptString(scanner, "Enter performer username: ", true);
                        entries = auditLog.getByPerformer(performer);
                        break;
                    case 3:
                        String action = ConsoleUtils.promptString(scanner, "Enter action (e.g., CREATE_USER): ", true);
                ...
                for (var entry : entries) {
                    rows.add(new String[]{
                        ...
                        FormatUtils.truncate(entry.action(), 15),
                        FormatUtils.truncate(entry.performer(), 15),
                        FormatUtils.truncate(entry.target(), 15),
            ...
            (scanner, system) -> {
                ...
                if (ConsoleUtils.promptYesNo(scanner, "\nExport report to file?")) {
                    ...
                    try {
                        ...
                        ConsoleUtils.printSuccess("Report exported to: " + filename);
                    } catch (Exception e) {
